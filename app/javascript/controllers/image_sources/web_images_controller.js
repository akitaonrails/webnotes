...
// Web Images Tab Controller
// Handles searching images via DuckDuckGo/Bing
...
export default class extends Controller {
  ...
  connect() {
    this.source = new WebImageSource()
  ...
  async search() {
    ...
    if (result.error) {
      ...
      if (this.hasGridTarget) {
        this.source.renderGrid(this.gridTarget, "click->web-images#select")
      }
    }
  ...
  select(event) {
    ...
    this.selectedImage = {
      ...
      source: item.dataset.source || "web"
    }
  ...
  async getImageUrl() {
    ...
    if (reuploadToS3) {
      const data = await this.source.uploadToS3(this.selectedImage.url, resizeRatio, this.csrfToken)
      return data.url
    }
